...
class ReplayBuffer:
    ...
    def sample_batch(self, env):
        ...
        xu_batch      = np.reshape(np.append(x_batch, u_batch), (env.pendulum.nx + env.pendulum.nu,1,self.batch_size))
        xu_next_batch = np.reshape(np.append(x_next_batch, u_next_batch),(env.pendulum.nx + env.pendulum.nu,1,self.batch_size))
